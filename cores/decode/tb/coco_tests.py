...
async def test_decode(dut, ins, ext=None, *, 
                      ...
                      read_c, src_c, set_pc, add_pc, inc_pc, cmp_b, 
                      pc_src, out_regs, alu_en, sh_off_imm, truth_table, 
                      alu_op, dst, mem_en, mem_write):
    ...
    assert dut.src_c.value == src_c
    assert dut.read_c.value == read_c
...
@cocotb.test
async def st3_works(dut):
    await test_decode(dut, 0x16AF,
                      imm_en=0,
                      arg_imm=0x00,
                      read_a=1,
                      arg_a=0x7,
